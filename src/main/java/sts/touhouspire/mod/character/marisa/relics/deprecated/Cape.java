package sts.touhouspire.mod.character.marisa.relics.deprecated;
...
public class Cape extends CustomRelic {
	...
	private static final String IMG = "marisa/img/relics/vCore.png";
	private static final String IMG_USED = "marisa/img/relics/usedvCore.png";
	private static final String IMG_OTL = "marisa/img/relics/outline/vCore.png";
	...
	public void onEnterRoom(AbstractRoom room) {
		...
		if ((room instanceof ShopRoom)) {
			MarisaModHandler.logger.info("Cape_1 : onEnterRoom : ShopRoom detected .");
			this.avail = true;
			flash();
			this.pulse = true;
		} else {
	...
	public void update() {
		...
		if ((this.counter <= 0) || (this.usedUp)) {
			MarisaModHandler.logger.info("Cape_1 : update : returning for being used");
			return;
		}
		if (!this.isObtained) {
			MarisaModHandler.logger.info("Cape_1 : update : returning for not obtained");
			return;
		}
		if ((this.RclickStart) && (InputHelper.justReleasedClickRight)) {
			if (this.hb.hovered) {
				MarisaModHandler.logger.info("Cape_1 : update : hovered");
				this.Rclick = true;
			}
			this.RclickStart = false;
		}
		if ((this.isObtained) && (this.hb != null) && (this.hb.hovered)
			...
			MarisaModHandler.logger.info("Cape_1 : update : right click detected");
			this.RclickStart = true;
		}
		if (this.Rclick) {
			MarisaModHandler.logger.info("Cape_1 : update : Calling OnRclick");
			this.Rclick = false;
			OnRclick();
		}
	}
