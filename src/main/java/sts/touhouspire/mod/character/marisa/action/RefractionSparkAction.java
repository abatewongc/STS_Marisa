...
import static sts.touhouspire.mod.character.marisa.patches.CardTagEnum.SPARK;
  ...
  public void update() {
    if ((this.duration == 0.1F) && (this.target != null)) {
      ...
      AbstractMonster mon = (AbstractMonster) this.target;
	    ...
	    MarisaModHandler.logger.info("RefractionSparkAction : calculating damage : " + this.info.base);
      ...
      if (tmp > 0) {
	      MarisaModHandler.logger.info("RefractionSparkAction : increasing damage : " + tmp);

        for (AbstractCard c : AbstractDungeon.player.hand.group) {
          if (c.hasTag(SPARK)) {
	          MarisaModHandler.logger.info("RefractionSparkAction : increasing damage for : " + c.cardID);
	          c.baseDamage += tmp;
            c.flash();
            c.applyPowers();
          }
	    ...
	    MarisaModHandler.logger.info("RefractionSparkAction : dealing damage : " + tmp);
