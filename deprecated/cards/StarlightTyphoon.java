...
public class StarlightTyphoon extends CustomCard {
  ...
  public static final String IMG_PATH = "marisa/img/cards/Defend.png";
  ...
  public void use(AbstractPlayer p, AbstractMonster m) {
    int cnt = 0;
    io.aleosiss.sts.character.marisa.logger.info("StarlightTyphoon : onUse");
    for (AbstractCard c : p.hand.group) {
      if ((c.type != CardType.ATTACK) && (c != this)) {
        io.aleosiss.sts.character.marisa.logger.info("StarlightTyphoon : exahsting : " + c.cardID);
        AbstractDungeon.actionManager.addToTop(
            new ExhaustSpecificCardAction(c, p.hand, true)
        );
        cnt++;
        io.aleosiss.sts.character.marisa.logger.info("StarlightTyphoon : counter : " + cnt);
      }
    }
    for (int i = 0; i < cnt; i++) {
      ...
      io.aleosiss.sts.character.marisa.logger.info("StarlightTyphoon : adding Spark : counter : " + cnt);
    }
  }
